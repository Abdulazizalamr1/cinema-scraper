...
HEADERS = {
    # Look like a normal Chrome browser. (No "bot" token — many WAFs block that.)
    "User-Agent": (
...
def fetch_html(date: dt.date | None = None, timeout: int = 25) -> str:
    ...
    with requests.Session() as s:
        ...
        # Visit the homepage first so we pick up any region/consent cookies,
        # then request showtimes (mimics a real browser session).
        try:
            s.get(BASE + "/", timeout=timeout, allow_redirects=True)
        except Exception:
            pass
        resp = s.get(SHOWTIMES_URL, params=params, timeout=timeout, allow_redirects=True)
        resp.raise_for_status()
        return resp.text


def diagnose(date: dt.date | None = None, timeout: int = 25) -> dict:
    """Fetch the page and report what we actually received, without parsing.

    Open /api/debug to see this. It tells us whether VOX served the real
    showtimes HTML or a block / challenge / region page.
    """
    params = {}
    if date:
        params["d"] = date.strftime("%Y%m%d")
    info: dict = {"chain": CHAIN}
    try:
        with requests.Session() as s:
            s.headers.update(HEADERS)
            try:
