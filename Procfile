...
def root():
    return {
        "status": "ok",
        "version": "v2-debug",
        "endpoints": ["/api/data", "/api/data?date=YYYY-MM-DD", "/api/debug"],
...
@app.get("/api/debug")
def api_debug():
    """Diagnostics: shows what each chain's site actually returns.
    Open this if /api/data comes back empty."""
    report = {}
    for name, _ in SCRAPERS:
        try:
            if name == "VOX":
                report["VOX"] = vox.diagnose()
            else:
                report[name] = {"note": "no diagnose() implemented"}
        except Exception as e:
